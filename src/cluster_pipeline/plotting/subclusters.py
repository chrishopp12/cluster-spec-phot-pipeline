...
def plot_subcluster_regions_and_histograms(
    ...
    **kwargs :
    ...
    for i, ax in enumerate(ax_list):
        ...
        # Each histogram panel corresponds to active_subs[i] (combined/dominant list),
        # not subclusters[i] (the full list)
        sub = active_subs[i]
        primary = sub.primary_bcg
        z_bcg = primary.z if primary is not None else None
        bcg_label = primary.label if primary is not None else sub.label
...
# ====================================================================
# GMM and velocity diagnostic plots (moved from subclusters/statistics.py)
# ====================================================================
...
def plot_gmm_histogram(z, valid_gaussians, plot_total=True, bins=60, n_inset_max=4, show=True, fit_info=None, save_path=None, save_plots=True, show_plots=True):
    """Plot field/cluster redshift histogram with GMM overlays and subcluster insets.
    ...
    n_inset_max : int
        Maximum number of insets to show.
    save_path : str or None
    ...
    Returns
    -------
    gaussian_colors : list[str]
        Colors assigned to each Gaussian component.
    """
    if len(z) == 0:
    ...
    inset_info = []
    color_cycle = cycle(plt.rcParams['axes.prop_cycle'].by_key()['color'][1:])
    gaussian_colors = []
    for idx, (z_low_fit, z_high_fit, mean, std, weight) in enumerate(valid_gaussians):
        ...
        ax.plot(x, gaussian, linestyle='dashed', linewidth=2, color=color, zorder=7,
    ...
    inset_positions = {
        1: [(0.65, 0.42)],
        2: [(0.65, 0.72), (0.65, 0.42)],
        3: [(0.08, 0.42), (0.65, 0.72), (0.65, 0.42)],
        4: [(0.08, 0.42), (0.65, 0.72), (0.65, 0.42), (0.65, 0.12)],
    }
    ...
    for inset_rank, i in enumerate(inset_indices):
        ...
        z_sub = z[sub_mask]
        if len(z_sub) == 0 or inset_rank >= len(positions):
            continue
...
def plot_stacked_velocity_histograms(vel_data, bins=25, color=None, combined_color='orange', with_all=False, save_path=None, save_plots=True, show_plots=True):
    """Plot stacked histograms of velocity data for each subcluster.

    Parameters
    ----------
    vel_data : list of (velocities, z_mean, sigma_v) tuples
        Per-subcluster velocity data.
    bins : int
        Number of histogram bins per subcluster.
    color : list or None
        Colors for subcluster histograms. Uses color cycle if None.
    combined_color : str
        Color for the combined histogram panel.
    with_all : bool
        If True, add a combined histogram as the last panel.
    save_path : str or None
        File path to save the figure.
    save_plots, show_plots : bool
        Plot display/save controls.
    ...
    for i, (velocities, z_mean, sigma_v) in enumerate(vel_data):
        ...
        ax.text(0.98, 0.96, textstr, transform=ax.transAxes, ha='right', va='top',
