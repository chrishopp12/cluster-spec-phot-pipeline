...
from cluster_pipeline.utils.resolvers import get_name, get_coordinates, get_redshift
from cluster_pipeline.io.catalogs import get_redseq_filename
from cluster_pipeline.io.images import is_fits_valid
...
# ------------------------------------
    ...
    # Navigate from src/cluster_pipeline/models/ up to the repo root
...
# ------------------------------------
        ...
        *,
        ...
        if not _is_valid_z(zmin) or not _is_valid_z(zmax):
            ...
            if not _is_valid_z(zmin):
                zmin = cluster_z - DEFAULT_Z_PAD
            if not _is_valid_z(zmax):
                zmax = cluster_z + DEFAULT_Z_PAD
    ...
    def config_path(self) -> str:
        """Path to this cluster's YAML config file."""
        return os.path.join(self.cluster_path, "config.yaml")

    def load_config(self) -> dict:
        """Load this cluster's YAML config."""
        from cluster_pipeline.config import load_config
        return load_config(self.cluster_path)

    def save_config(self, config: dict) -> None:
        """Save config to this cluster's YAML file."""
        from cluster_pipeline.config import save_config
        save_config(self.cluster_path, config)
    ...
    def get_optical_image(
        ...
        fov_eff = float(fov) if fov is not None else float(self.fov or DEFAULT_FOV_ARCMIN)
        ra_offset_eff = float(ra_offset) if ra_offset is not None else float(self.ra_offset or DEFAULT_RA_OFFSET_ARCMIN)
        dec_offset_eff = float(dec_offset) if dec_offset is not None else float(self.dec_offset or DEFAULT_DEC_OFFSET_ARCMIN)
...
# ------------------------------------
    ...
    def _init_paths(self) -> None:
        ...
        self._xray_file = os.path.join(self._xray_path, DEFAULT_XRAY_FILENAME)
    ...
    def _populate_from_defaults(self, *, verbose: bool = True) -> None:
        ...
        defaults: dict[str, Any] = {
            ...
            'contour_levels': "0.5,0,12",  # Keep as string for backward compat
